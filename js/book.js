$(function(){
	...
	for (var i = 0; i < $('.runPage').length; i++) {
		...
		$('.runPage').eq(i).children('div').css('z-index',8-2*i);
		$('.runPage').eq(i).children('img').css('z-index',7-2*i);
	};
	...
	function zIndexLast(index,element){
		...
		setTimeout(function(){
			element.css('z-index',8-2*index);
			element.children('div').css('z-index',8-2*index);
			element.children('img').css('z-index',7-2*index);
		},1000);
	}
});